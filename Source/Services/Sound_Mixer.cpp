...
Sound_Mixer::Sound_Mixer()
{
    ...
    m_thread = std::jthread(std::bind_front(&Sound_Mixer::ProcessAudioCommands, this));
}

Sound_Mixer::~Sound_Mixer() noexcept
{
...
void Sound_Mixer::PlayAudio(uint32_t audioID)
{
...
void Sound_Mixer::PlayTrack(std::string const& tag)
{
    std::scoped_lock const lock(m_mutex);
...
void Sound_Mixer::LoadAudio(std::string const& path, uint32_t audioID)
{
    std::scoped_lock const lock(m_mutex);
...
void Sound_Mixer::LoadPersistentAudioTrack(std::string const& path, std::string const& audioTag)
{
    std::scoped_lock const lock(m_mutex);
...
void Sound_Mixer::ProcessAudioCommands(std::stop_token stopToken)
{
    while (true)
    {
...
auto Sound_Mixer::RequestTrack() -> MIX_Track*
{
...
void Sound_Mixer::PlaySingleTimeAudio(uint32_t audioID)
{
    // Doesn't need a lock, is only called from ProcessCommands which already locks the mutex
...
void Sound_Mixer::PlayTaggedTracks(std::string const& tag) const
{
    // Doesn't need a lock, is only called from ProcessCommands which already locks the mutex
    MIX_PlayTag(m_mixer, tag.c_str(), 0);
}

void Sound_Mixer::SetTrackAudioNull([[maybe_unused]] void* pUserData, MIX_Track* track)
{
